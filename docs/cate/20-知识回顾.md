...
使用`eventBus`进行跨组件触发事件，进而传递数据 eventbus 的思想是什么
...
- ssr渲染 以字符串方式渲染
- proxy替换了之前的defineProterty，解决了什么问题
- vite
- ts
- tree shaking
...
它会去编译你的代码吗，vite 引用 commonjs 的包的时候怎么处理
...
按去a最便宜的排序，然后前面n个就是去a最便宜的，后面n个就是去b最便宜的

```js
var twoCitySchedCost = function(costs) {
    var len = costs.length/2
    var result = 0
...
先将所有人去b的总费用算出来，再将每个人a-b的差值算出来排序，差值越小的说明去a越便宜，前n个去a。总值加上前n个人的差值即可
...
const unite = new Set(arr1.filter(item => set2.has(item)))
console.log(unite);
```
